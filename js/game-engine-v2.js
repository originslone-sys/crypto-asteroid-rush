/* ============================================
   UNOBIX - Game Engine v2.0
   File: js/game-engine-v2.js
   Baseado em game-engine.js v8.0
   v2: Usa getCurrentModeConfig(), suporta treino
   ============================================ */
...
// v2: Se treino, não envia ao servidor
// ============================================
async function gameOver() {
    ...
    // v2: Em modo treino, não comunica com o servidor
    if (window.isTrainingMode) {
        console.log('🏋️ Treino - game over local (sem servidor)');
    } else {
...
function startSpawnTimer() {
    ...
    const interval = modeConfig.spawn_interval || 500;
    const maxAsteroids = modeConfig.max_asteroids || 10;
...
async function endGame() {
    ...
    // v2: Em modo treino, não comunica com o servidor
    if (window.isTrainingMode) {
        console.log('🏋️ Treino finalizado - sem comunicação com servidor');
        if (typeof showNotification === 'function') {
            showNotification('🏋️ TREINO', 'Treino finalizado!', true);
        }
    } else {
...
console.log('📦 game-engine-v2.js v2.0 carregado (suporte a treino + modos)');
