...
// v6.0 - Sistema de alerta automático para earnings > $0.06
// ============================================
...
try {
    ...
    // Verificar ban
    $stmt = $pdo->prepare("SELECT is_banned, ban_reason FROM players WHERE wallet_address = ?");
    $stmt->execute([$wallet]);
    $player = $stmt->fetch();
    ...
    // ATUALIZAR SALDO
    // ============================================
    $stmt = $pdo->prepare("SELECT id, balance_usdt FROM players WHERE wallet_address = ?");
    $stmt->execute([$wallet]);
    $playerData = $stmt->fetch();
    
    $newBalance = 0;
    if ($playerData) {
        $newBalance = (float)$playerData['balance_usdt'] + $finalEarnings;
    ...
    secureLog("SESSION_END | Session: {$sessionId} | Earnings: \${$finalEarnings} | Alert: {$alertLevel}");
    
    echo json_encode($response);
    
} catch (Exception $e) {
    secureLog("END_ERROR | Session: {$sessionId} | Error: " . $e->getMessage());
    echo json_encode(['success' => false, 'error' => 'Erro ao finalizar sessao']);
}
?>