...
// Arquivo: api/withdraw.php v4.0
// Apenas PIX, sem PayPal/USDT
// ============================================
...
try {
    ...
    $stmt = $pdo->prepare("SELECT id, balance_brl FROM users WHERE google_uid = ? FOR UPDATE");
    ...
    $stmt = $pdo->prepare("
        INSERT INTO withdrawals (
            player_id, google_uid, amount_brl,
            payment_method, payment_details,
            status, created_at
        ) VALUES (?, ?, ?, 'pix', ?, 'pending', NOW())
    ");
    $stmt->execute([(int)$player['id'], $googleUid, $amount, $details]);

    $withdrawalId = (int)$pdo->lastInsertId();

    // Debitar saldo
    $newBalance = $balance - $amount;
    $stmt = $pdo->prepare("UPDATE users SET balance_brl = ? WHERE id = ?");
    $stmt->execute([$newBalance, (int)$player['id']]);

    // Atualizar total_withdrawn_brl se coluna existir
    try {
    ...
    secureLog("WITHDRAW_REQUEST | UID: {$googleUid} | Amount: {$amount} | #{$withdrawalId}");
